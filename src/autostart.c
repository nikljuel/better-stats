...
static const char handler_script[] =
    ...
    "\n"
    "started=$(date +%s 2>/dev/null) || started=0\n"
    "case \"$started\" in ''|*[!0-9]*) started=0 ;; esac\n"
    "read_proc_start() {\n"
    "    stat=$(cat \"/proc/$$/stat\" 2>/dev/null) || exit 1\n"
    "    rest=${stat##*\\) }\n"
    "    [ \"$rest\" != \"$stat\" ] || exit 1\n"
    "    set -- $rest\n"
    "    [ \"$#\" -ge 20 ] || exit 1\n"
    ...
    "    printf '%s' \"$value\"\n"
    "}\n"
    "proc_start=$(read_proc_start) || proc_start=\"\"\n"
    "pid_tmp=\"" READER_PIDFILE ".$$\"\n"
    "{\n"
    ...
    "    fi\n"
    "    printf '%s\\n' \"$1\"\n"
    "} > \"$pid_tmp\" && mv -f \"$pid_tmp\" \"" READER_PIDFILE "\"\n"
